...
- 📗 **TOEIC** (910개): 주제별 TOEIC 빈출 어휘 (`build_toeic.py`로 생성, 내 단어장과 중복 제외)
- 💬 **문장** (209개): 상황별 실전 회화 문장 (`build_sentences.py`로 생성) — 한국어를 보고
  영어로 말해보는 방식으로 학습하고, 퀴즈는 🧩 문장 조립 퍼즐 + 한→영 선택형
- ⭐ **내 카드**: 홈의 ➕ 카드 추가로 직접 넣은 단어/문장 (넷플릭스 대사 등).
  영어만 입력하면 AI가 뜻·발음·예문을 자동 생성하고, `영어 | 뜻` 형식은 AI 없이 즉시 추가.
  카드는 학습 기록과 함께 Gist 동기화에 포함됨
- **전체**: 단어 덱 합산 (문장은 별도 탭)
...
build_sentences.py  실전 회화 문장 + 데이터 생성 스크립트
